...
from unraid_mcp.tools._array import _ARRAY_LEGACY_MUTATIONS, _ARRAY_MUTATIONS, _ARRAY_QUERIES
...
def public_operation_dicts(*, include_legacy: bool = True) -> Iterable[tuple[str, dict[str, str]]]:
    ...
    be excluded when validating against the current vendored SDL.
    """
    ...
    return (
        ...
        *((("array", _ARRAY_LEGACY_MUTATIONS),) if include_legacy else ()),
        ("disk", _DISK_QUERIES),
...
def schema_operation_dicts() -> Iterable[tuple[str, dict[str, str]]]:
    ...
    yield from public_operation_dicts(include_legacy=False)
...
def dispatch_operation_cases(*, include_legacy: bool = True) -> list[tuple[str, str, str]]:
